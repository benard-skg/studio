...
export default function SignInPage() {
  ...
  useEffect(() => {
    // This effect handles redirection if the user is ALREADY logged in when they visit this page.
    if (!authLoading && user) {
  ...
  // The GoogleSignInButton will now handle its own redirection AFTER successful sign-in.
      ...
      <main className="flex-grow flex items-center justify-center p-4">
        <Card className="w-full max-w-sm shadow-xl border-border">
          ...
          <CardContent className="space-y-4">
            {/* GoogleSignInButton now handles its own post-login navigation */}
            <GoogleSignInButton /> 
